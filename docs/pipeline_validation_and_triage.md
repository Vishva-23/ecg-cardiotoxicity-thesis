...
All figures below are in [`docs/figures/`](figures/). (The full `outputs/` folder is
git-ignored because it is regenerated by the notebooks; these are the figures the
report cites.)

**Pipeline validation — raw → filtered → feature extraction**

![Validation overlay, clean animal 201](figures/overlay_validation_A201.png)

![Validation overlay, problem animal 111](figures/overlay_validation_A111.png)

**Filter verification — FFT power spectrum, before vs after (overlaid)**
...
Also: [A122](figures/clean_rawvsfilt_A122.png), [A116](figures/clean_rawvsfilt_A116.png).

**Recovery checks** — original vs relocated window

![Recovery check A123](figures/recovered_A123.png)

Also: [A126 (borderline)](figures/recovered_A126.png).

**3D PCA and nonlinear (t-SNE) embedding** — supports
[why_supervised_is_required.md](why_supervised_is_required.md)

![3D PCA and t-SNE](figures/14_pca3d_tsne.png)

**Not included in the repository:** per-animal raw-vs-filtered figures for all 34
flagged animals (`outputs/problem_animals/`, ~5.8 MB) are regenerated by the
notebooks rather than committed.
